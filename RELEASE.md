...
The trigger is a submission, not a date: **v0.3.0 goes out when OUT-005 (The Second Gate, CLSR) has been submitted.** Until then `--prepare` refuses: a release that lifts no embargo needs an explicit `--code-only`, and that refusal is the embargo of Architecture §9.3 doing its job. `--force` overrides the other blocked checks but can never lift an embargo, because what the embargo releases is derived from the Hub output status alone.
...
4. **Prepare.** `node scripts/release.mjs --prepare --version=0.3.0 --title="Second Gate published"`. This bumps `package.json` and `CITATION.cff`, dates the CHANGELOG heading, adds a bullet recording the embargo lift, and flips the current Module 02 snapshot (2026-09-07) from `review` to `published` with a one-line diff. The superseded 2026-08-31 keeps its status: publishing a snapshot that a later one replaces would assert something nobody claims, and it carries the v0.1.0 DOI already.
5. **Check what you are about to publish.** `npm run validate` (0 errors), `npm run build`, `npm run serve`, and look at the Module 02 snapshot page for 2026-09-07: the draft banner must be gone and the citation block must show the DOI.
