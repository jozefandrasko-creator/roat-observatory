...
2. Hub hygiene backlog (not blocking the Observatory). Done on 2026-09-06 by prompts 10 and 11: Legal Status on 56 legal-type records, 14 placeholder titles resolved, 0142 rejected as a duplicate of 0120. 0174 (Arizona § 28-9702) resolved by prompt 15. Still open: 0245 and 0263 SSRN metadata — SSRN sits behind a bot check that blocked Claude (cloud and browser pane) and ChatGPT (prompt 18, 403) alike, so the author opens the two abstract pages and pastes title, authors and posting date into Blok C of `Claude outputs/ROAT-prompt-18-outputs-a-ssrn.md` (or prompt 15 Blok B); 0318–0321 public fields (kept in Review on purpose); 0262 Official Source points to the analysed Croatian gazette rather than the ROAT analysis itself (author's choice pending a public URL).
