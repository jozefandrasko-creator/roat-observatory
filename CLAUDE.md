...
- Landscape map: `content/landscape/<date>.json` (76 boxes, 12 pillars, 5 layers, 6 status classes) renders at `/map/`; imported once from the ROAT Regulatory Map workbook v0.7.1 by `Claude outputs/tools/landscape-from-xlsx.mjs`. Boxes reference Hub records and link to `/sources/<id>/` only when the record is Published, so the map doubles as a publication queue: 55 of the 88 referenced records are still unpublished.
