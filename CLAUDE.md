...
1. **Release v0.3.0 is prepared and waiting for one fact: OUT-005 submitted.** Everything mechanical is in `scripts/release.mjs`: `npm run release-check` is the preflight (embargo per module, seven repository checks, and exactly which files would change), `--prepare --version=X.Y.Z --title="..."` bumps package.json and CITATION.cff, dates the CHANGELOG heading, records the embargo lift and flips the released snapshots with a one-line diff each, and `--doi=10.5281/zenodo.NNNNNNNN` writes a minted DOI only into published snapshots that have none. Order of operations, including the Zenodo steps, is in RELEASE.md under "The next release, v0.3.0". The Hub side is `Claude outputs/ROAT-prompt-24-OUT-005-submitted.md`; draft notes are `Claude outputs/release-notes-v0.3.0.md`. Modules 01 and 03 stay embargoed until OUT-013 and OUT-002 are submitted.
...
- Verifying a legal date against EUR-Lex: the base and consolidated texts do NOT show corrigenda. Open the ALL view (`legal-content/EN/ALL/?uri=CELEX:...`) and read the "Corrected by" relationship before concluding the Hub is wrong. On 2026-09-06 a verification pass declared ROAT-2026-0320 wrong about the Product Liability Directive applying after 8 December 2026; the corrigendum of 7 May 2026 (OJ L, 2026/90364) makes the Hub right and the base text stale.
...
- Navigation layer: `content/guide/{decision-tree,journey,slovakia-stack,interfaces,governance}.json` (the last two imported by `Claude outputs/tools/guide-from-xlsx.mjs`), plus `/method/roles/` which is generated from the concepts and the Module 03 matrix with no content file of its own render at `/map/decision-tree/`, `/map/journey/`, `/map/slovakia/` and (Slovak) `/sk/slovensko-v-stacku/`. Imported from the workbook; `status` is `draft` until the author approves. The Slovak stack page shares one JSON with the English one (`*_sk` fields).
