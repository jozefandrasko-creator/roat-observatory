...
- Landscape map: `content/landscape/<date>.json` (76 boxes, 12 pillars, 5 layers, 6 status classes) renders at `/map/`; imported from the ROAT Regulatory Map workbook v0.7.1 by `Claude outputs/tools/landscape-from-xlsx.mjs`. Once the Hub carries a `Regulatory Map` sheet (prompt 21), `hub-sync` writes `data/hub/landscape.json` and the build prefers it. The sheet was created on 2026-09-06 (CHK-31 0, CHK-32 8) but from a pre-correction copy of the data block; prompt 23 realigns eight cells (BOX-018, BOX-019, BOX-041 status and note; BOX-031 and BOX-033 first URL). **Do not run the sync until prompt 23 is reported done** — it would overwrite published corrections. Whenever a prompt carries a generated data block, re-check the sheet against the repo file before the first sync: the author may have pasted an earlier copy. Boxes reference Hub records and link to `/sources/<id>/` only when the record is Published, so the map doubles as a publication queue: 55 of the 88 referenced records are still unpublished.
